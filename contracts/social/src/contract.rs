...
use crate::msg::{ExecuteMsg, InstantiateMsg, QueryMsg, TradeInfo};
use crate::state::{Config, CONFIG, TRADER_PROFILES, TraderProfile};
...
pub fn execute_follow(
    ...
    if !profile.followers.iter().any(|f| f.address == info.sender) {
        ...
        profile.followers.push(crate::state::Follower {
            address: info.sender.clone(),
            stake_amount: Uint128::new(stake_amount),
        });
        
        // Update total stake
        profile.total_stake = profile.total_stake.checked_add(Uint128::new(stake_amount))?;
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            ...
            attr("trader", trader),
            attr("stake_amount", stake_amount.to_string()),
        ]))
}
...
pub fn execute_copy_trade(
    ...
    let profit = Uint128::new(trade.expected_profit);
    
    // Calculate fees
    let config = CONFIG.load(deps.storage)?;
    let trader_fee = profit.multiply_ratio(config.trader_fee_percent, 100);
    let treasury_fee = profit.multiply_ratio(config.treasury_fee_percent, 100);
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            attr("method", "copy_trade"),
            attr("sender", info.sender),
            attr("trader", trader),
            attr("trade_type", trade.trade_type),
            attr("token_pair", trade.token_pair),
            attr("amount", trade.amount.to_string()),
            attr("profit", profit.to_string()),
            attr("trader_fee", trader_fee.to_string()),
            attr("treasury_fee", treasury_fee.to_string()),
        ]))
}
...
fn query_trader_profile(deps: Deps, address: String) -> StdResult<Binary> {
    ...
    let profile = TRADER_PROFILES.may_load(deps.storage, &addr)?
        .unwrap_or_else(|| TraderProfile {
            address: addr,
            followers: vec![],
            total_stake: Uint128::zero(),
