...
func (k KalefiKeeper) StoreTradeEvent(ctx sdk.Context, trader string, amount sdk.Uint) error {
	...
	if !params.TradeEnabled {
		return sdkerrors.Wrap(sdkerrors.ErrUnauthorized, "trading is currently disabled")
	}

	// Check if the amount is within the maximum allowed
	maxAmount, err := sdk.ParseUintFromString(params.MaxTradeAmount)
	if err != nil {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "invalid max trade amount in params")
	}
	
	if amount.GT(maxAmount) {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidRequest, "trade amount %s exceeds maximum allowed %s", amount, maxAmount)
	}
